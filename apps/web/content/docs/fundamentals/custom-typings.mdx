...
Affected columns in `tables.ts` will use your validators. Damian output wraps them with `s.fromStandard(...)`, while Drizzle output applies the inferred type via `.$type<...>()`.
