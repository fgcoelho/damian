---
title: Quickstart
description: Install and run your first migration
---

import { File, Folder, Files } from "fumadocs-ui/components/files"

## Install

<Tabs groupId='package-manager' persist items={['npm', 'pnpm', 'yarn']}>

```bash tab="npm"
npm install -D damian && npm install @damiandb/pg
```

```bash tab="pnpm"
pnpm add -D damian && pnpm add @damiandb/pg
```

```bash tab="yarn"
yarn add -D damian && yarn add @damiandb/pg
```

</Tabs>

---

## Configure

```ts title="damian.config.ts"
import { config } from "damian"

export default config({
    driver: "postgres",
    output: {
        kind: "damian",
        casing: "preserve",
    },
    root: "./damian",
    env: ".env",
    url: process.env.DATABASE_URL
})
```

---

## Create migration

```bash
npx damian new create_users_table
```

Edit the file under `./damian/.migrations/`:

```sql title="./damian/.migrations/20240101000000_create_users_table.sql"
-- migrate:up
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL,
    email TEXT UNIQUE NOT NULL,
    created_at TIMESTAMP DEFAULT NOW()
)

-- migrate:down
DROP TABLE users
```

---

## Apply migration

```bash
npx damian migrate
```

---

## Generate types

```bash
npx damian generate
```

Produces `tables.ts`, `typings.ts`, and `.generated/db.sql`.

Set `output.kind: "drizzle"` to generate Drizzle tables instead of Damian tables. Use `output.casing: "preserve" | "camel" | "snake"` to control generated property names; default is `"preserve"`.

---

## Setup path alias

<Tabs groupId='languages' persist items={['typescript', 'javascript']}>

```json tab="typescript" title="tsconfig.json"
{
  "compilerOptions": {
    "baseUrl": ".",
    "paths": {
      "tables": ["./damian/.generated/tables.ts"]
    }
  }
}
```

```json tab="javascript" title="package.json"
{
  "imports": {
    "#tables": "./damian/.generated/tables.js"
  }
}
```

</Tabs>

---

## Create database client

```ts title="db.ts"
import { createDb, createSQL } from '@damiandb/pg'

export const db = await createDb({
    connectionString: process.env.DATABASE_URL as string
})

export const sql = createSQL()
```

---

## Write a query

```ts title="index.ts"
import { db, sql } from './db'
import { UsersTable } from 'tables'

const { rows } = await db.query(
    sql(UsersTable)`SELECT * FROM ${UsersTable} WHERE ${UsersTable.name} = ${"Alice"}`
)

const user = rows[0]
```

---

## Update gitignore

```txt title=".gitignore"
./damian/.generated
```
